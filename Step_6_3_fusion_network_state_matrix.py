...
data = "/home/fancywu/Desktop/meterial/test_file"
save_dst = "/home/fancywu/Desktop/meterial/Test_early_fusion_state_matrix"
res_dir_ = "/home/fancywu/Desktop/meterial/result/multi/"
...
#load_path = args.path
load_path = '/home/fancywu/Desktop/meterial/result/multi/37.8_2018.08.21.13.46.54/'
######################################################################
    ...
    if condition:   #wudi only used first 650 for validation !!! Lio be careful!
        ...
        for batchnumber in xrange(video.shape[0]/batch.micro):
            ...
            if not isinstance(Mean_CNN,int):
                x_.set_value(normalize(video_temp.astype("float32"), Mean_CNN, Std_CNN).astype("float32"),borrow=True)
            else:
                x_.set_value(normalize(video_temp, Mean_CNN, Std_CNN).astype("float32"),borrow=True)
            if not isinstance(Mean_skel,int):
                x_skeleton_.set_value(normalize(skel_temp.astype("float32"),Mean_skel, Std_skel).astype("float32"), borrow=True)
            else:
...
print "totally time used:%f"%(time() - pro_start_time)    
